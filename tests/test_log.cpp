...
TEST_CASE("replay reproduces book state hash") {
  LoggedBook logged;
  logged.add_limit(buy_at(1, 100, 10, 1), 1);
  logged.add_limit(sell_at(2, 100, 4, 2), 2);
  logged.add_limit(buy_at(3, 99, 5, 3), 3);
  logged.cancel(1, 4);
  logged.add_limit(sell_at(4, 101, 3, 5), 5);

  const auto original_hash = logged.book().state_hash();
  const auto replayed      = replay(logged.log());
  CHECK(replayed.state_hash() == original_hash);
