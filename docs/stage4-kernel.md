...
**Phase 3 の現状 (プレースホルダ)。** `ContinuousMarket` は `AllocationRule` を
メンバとして保持するが、マッチングは `OrderBook` に委譲している。
`PriceTimePriority` は空の派生クラスであり、差し替えても挙動は変わらない。
要件 2 の「差し替え可能」は **未達** — プロラタを 1 つ実装して初めて
インターフェースの妥当性が検証できる。Stage 4 の到達点(再現性)には
配分規則の完成は含めない。
